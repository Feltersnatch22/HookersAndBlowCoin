...
if [[ ${OS} == "windows" ]]; then
    ...
    if [[ -e ${GITHUB_WORKSPACE}/dist/HookersAndBlow-Qt.app/Contents/MacOS/install_cli.sh ]]; then
        chmod +x ${GITHUB_WORKSPACE}/dist/HookersAndBlow-Qt.app/Contents/MacOS/install_cli.sh
    fi
    ...
    ${GITHUB_WORKSPACE}/depends/x86_64-apple-darwin14/native/bin/dmg dmg "HookersAndBlow-Core.dmg" ${RELEASE_LOCATION}/${DISTNAME}-osx-unsigned.dmg
