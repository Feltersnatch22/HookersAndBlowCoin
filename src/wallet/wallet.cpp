...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetHNB) { // Looking for HNB Tx OutPoints Only
                    if (fHNBLimitHit) // We hit our limit
                        continue;

                    // We only want HNB OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /** HNB START */
                    // Check to see if you need to make an asset data outpoint OP_HNB_ASSET data
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** HNB START */ /** HNB END */
                    {
