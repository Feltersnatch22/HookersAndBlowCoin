...
void static HookersAndBlowMiner(const CChainParams& chainparams)
{
    LogPrintf("HookersAndBlowMiner -- started\n");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("HookersAndBlowMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("HookersAndBlowMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("HookersAndBlowMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("HookersAndBlowMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("HookersAndBlowMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateHookersAndBlows(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&HookersAndBlowMiner, boost::cref(chainparams)));
    }
