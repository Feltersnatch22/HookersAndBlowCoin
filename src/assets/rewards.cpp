...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for HNB
    ...
    if (p_rewardSnapshot.strDistributionAsset != "HNB") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is HNB with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using HNB differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "HNB") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse HookersAndBlow address (already validated during ownership snapshot creation)
