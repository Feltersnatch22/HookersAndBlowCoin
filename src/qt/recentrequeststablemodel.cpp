    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return HookersAndBlowUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, HookersAndBlowUnits::separatorNever);
            else
                return HookersAndBlowUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+HookersAndBlowUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
