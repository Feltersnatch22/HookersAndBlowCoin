...
//{
//    columns[Amount] = HookersAndBlowUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, HookersAndBlowUnits::SeparatorStyle separators) const
{
    QString str;
    switch(wtx->type) {
        case TransactionRecord::Issue:
        ...
        default:
            {
            str = HookersAndBlowUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit,
                                             false, separators);
            } break;
    }
    if (showUnconfirmed) {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, HookersAndBlowUnits::separatorAlways);
        case AssetName:
            if (rec->assetName != "HNB")
               return QString::fromStdString(rec->assetName);
            else
               return QString(HookersAndBlowUnits::name(walletModel->getOptionsModel()->getDisplayUnit()));
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, HookersAndBlowUnits::separatorNever));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, HookersAndBlowUnits::separatorNever);
    case AssetNameRole:
        {
            QString assetName;
            if (rec->assetName != "HNB")
               ...
               assetName.append(QString(HookersAndBlowUnits::name(walletModel->getOptionsModel()->getDisplayUnit())));
            return assetName;
        }
    case StatusRole:
...
QVariant TransactionTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if(orientation == Qt::Horizontal)
    {
        ...
        } else if (role == Qt::ToolTipRole)
        {
            ...
            case AssetName:
                return tr("The asset (or HNB) removed or added to balance.");
            }
        }
    }
    return QVariant();
