    ...
    if (nNet > 0 || wtx.IsCoinBase())
    {
        ...
        for(unsigned int i = 0; i < wtx.tx->vout.size(); i++)
        {
            ...
            if(mine)
            {
                ...
                {
                    // Received by HookersAndBlow Address
    ...
    else
    {
        ...
        else if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++)
            {
                ...
                {
                    // Sent to HookersAndBlow Address
        ...
        if(fIsSingleSign)
        {
            ...
            else //We were the ones executing the transaction
            {
                ...
                for(const CTxOut &txout : wtx.tx->vout)
                {
                    if(wallet->IsMine(txout))
                    {
                        //If we sent assets, we need to see if we were sent assets or HNB in return
                        if(txout.scriptPubKey.IsAssetScript())
                        {
            ...
            if(fSentAssets && fRecvAssets) {
                ...
                std::string asset_qty_format = HookersAndBlowUnits::formatWithCustomName(QString::fromStdString(sentType), sentAmount, 2).toUtf8().constData();
                ...
                std::string asset_qty_format = HookersAndBlowUnits::formatWithCustomName(QString::fromStdString(sentType), sentAmount, 2).toUtf8().constData();
                sub.assetName = strprintf("HNB (%s %s)", TransactionView::tr("Sold").toUtf8().constData(), asset_qty_format);
            } else if (fRecvAssets) {
                //Buy!
                //Total price paid, need to use net calculation when we executed
                sub.credit = (mine ? -myProvidedInput.nValue : nNet);
                std::string asset_qty_format = HookersAndBlowUnits::formatWithCustomName(QString::fromStdString(recvType), recvAmount, 2).toUtf8().constData();
                sub.assetName = strprintf("HNB (%s %s)", TransactionView::tr("Bought").toUtf8().constData(), asset_qty_format);
            } else {
