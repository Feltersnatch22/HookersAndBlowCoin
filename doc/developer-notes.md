...
The HookersAndBlowCoin Doxygen documentation can be generated by running `make docs` from the top-level 
...
HookersAndBlow Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- HookersAndBlowMiner : Generates ravens (if wallet is enabled).
...
However, in open source software such as HookersAndBlow Core, where everyone uses
...
pay attention to for reviewers of HookersAndBlow Core code.

General HookersAndBlow Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: HookersAndBlow Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against HookersAndBlowCoin so that they can be integrated
