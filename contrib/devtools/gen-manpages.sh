...
HNBVER=($($RAVENCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $RAVEND $RAVENCLI $RAVENTX $RAVENQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${HNBVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${HNBVER[1]}//g" ${MANDIR}/${cmdname}.1
done
